...
from models.TED_model import *
from sklearn.metrics import mean_absolute_percentage_error
import logging
logging.basicConfig(filename = "TED.log", filemode='w', level = logging.DEBUG)
random.seed('a')
...
#         super(DAN, self).__init__(opt)
    ...
    def generate_single_val_rmse(self, min_RMSE=500):
        ...
        if total < min_RMSE:
            ...
            #save_model
            encoder_name = basic_model_path + '_' + watersheds + str(self.TrainEnd) + "_encoder.pt"
            decoder_name = basic_model_path + '_' + watersheds + str(self.TrainEnd) + "_decoder.pt"
    ...
    def model_load(self):
        ...
        self.encoder = torch.load(encoder_name)
        self.decoder = torch.load(decoder_name)
    ...
    def train(self):
        ...
        for epoch in range(num_epochs):
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                aa = torch.cat([x_train[:,-1:,:], x_train[:,-1:,:]], dim = 1)
                aa = torch.cat([aa, aa], dim=1)

                # compute ground segment label (Ensemble times=4, 16, 32, 96, 288)
                y_train_s = y_train.view(y_train.size(0), 4, 6*4*3)
                seg_label_g0 = torch.mean(y_train_s, dim=2) # aggr level 0 
                
                y_train_s = y_train.view(y_train.size(0), 16, 3*2*3)
                seg_label_g1 = torch.mean(y_train_s, dim=2) # aggr level 1
                
                y_train_s = y_train.view(y_train.size(0), 32, 3*3)
                seg_label_g2 = torch.mean(y_train_s, dim=2) # aggr level 2            

                y_train_s = y_train.view(y_train.size(0), 96, 3)
                seg_label_g3 = torch.mean(y_train_s, dim=2) # aggr level 3
                ...
                loss2 = self.criterion_KL(F.log_softmax(out2,dim=1), F.softmax(seg_label_g2,dim=1)) 
                loss3 = self.criterion_KL(F.log_softmax(out3,dim=1), F.softmax(seg_label_g3,dim=1))
                loss4 = self.criterion_KL(F.log_softmax(out4,dim=1), F.softmax(seg_label_g4,dim=1))
                
                loss5 = self.criterion(out4, seg_label_g4)  
                l_w = max(-1 * np.exp(epoch/45) + 2, 0)
                loss = l_w * 100 *( loss0 + loss1 + loss2 + loss3 ) + loss5
